...
def main():
    ...
    if st.button("Submit"):
        ...
        if st.button("Go!"):
            ...
            # 3) button
            if st.button("Press here to proceed to visualization"):
                if api_option == 'Run NER with WordLift':
                ...
                if st.sidebar.button("Press here to continue"):
                    @st.cache(allow_output_mutation=True, show_spinner=False)
    ...
    # 3- Intent by Entity and Search Volume
    ...
    # 4- Intent by Entity and Competition
