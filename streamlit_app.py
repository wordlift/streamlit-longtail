...
# these don't work unfortunatily, require a lot of changes delaying this for now
# from apps import home, wordlift_api, spacy_api, file_download
# from apps.home import RestClient
# from apps.home import autocomplete, generate_keywords, st_html
# from apps.wordlift_api import wl_nlp, string_to_entities_wl
# from apps.spacy_api import string_to_entities_sp
# from apps.file_download import download_button
...
if api_option == 'Run NER with WordLift':
    # here we still need to handle the error if there is no key added we shall not run the entity extraction or it will throw another error
...
# text inputs
st.markdown(""" <span style="color:green"> * Please type at least two ideas. </span> """, unsafe_allow_html=True)
...
## maybe these two functions should stay here, but the others can be out, idk still need to be figured out
...
def main():
    ...
    if api_option == 'Run NER with WordLift':
        ...
        if st.button("Go!"):
            ...
            if first_idea:
                # 2- if users enter only one idea then press go, they get another error.
                if not (second_idea or third_idea):
                    st.error("You have typed only one idea! Please type at least two ideas, then press GO!")
                elif second_idea:
                    # 3- if users enter only two ideas, then keyword_list will consist of only two ideas.
                    if not third_idea:
                        st.success("success!")
                        keyword_list = first_idea + ', ' + second_idea
                    # 4- if users enter three ideas, then keyword_list will consist of three ideas
                    elif third_idea:
                        st.success("success!")
                    ...
                    for i in range(100):
                        ...
                        time.sleep(0.1) # Pretend we're doing some computation that takes time.

                        # might need try except else here
                    ...
                    for i in range(100):
                        ...
                        response = client.post(WL_key, "/keywords_data/google/search_volume/live", post_data)
        ...
        if st.button("Go!"):
            ...
            for i in range(100):
                ...
                for index, row in df.iterrows():
                    data_x = string_to_entities_sp(df['queries'][index]) # extracting entities
                    if len(data_x[0]) is not 0: # make sure there are entities
                        df.at[index,'entities'] = data_x[0]
                        df.at[index,'types'] = data_x[1]
                    else:
