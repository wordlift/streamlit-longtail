...
PAGE_CONFIG = {
    ...
    "page_icon":"static/img/fav-ico.png",
    "layout":"centered"
    }
...
def set_png_as_page_bg(png_file):
    ...
    return
set_png_as_page_bg('static/img/pattern.png')
...
if size_navigation == 'Small': x = 10
elif size_navigation == 'Medium': x = 40
elif size_navigation == 'Large': x = 100
...
st.sidebar.image("static/img/logo-wordlift.png", width=200)
...
if api_option == 'Run NER with WordLift':
    ...
    st.sidebar.info("Please provide your WordLift key to proceed.")
...
if WL_key_ti:
    ...
    # st.sidebar.markdown(""" <span style="color:red"> Please enter your WordLift key </span> """, unsafe_allow_html=True)
    st.stop()
...
def main():
    ...
    if api_option == 'Run NER with WordLift':
        ...
        if st.button("GO!"):
            ...
            # we might not need all this inside the loop
            for i in range(100):

                progress_bar.progress(i + 1) # Update progress bar.
                # we can here also change 0.1 to some higher value
                time.sleep(0.1) # Pretend we're doing some computation that takes time.
                ...
                # visualizing
                # here i assume there are a lot of lines that can be removed
                ...
                # a toggle to switch between the two visualizations
                # the treemap is the end result and should be wider in size
                # st.beta_expander
