...
def app():
    ...
    language = w_app.lang_option
    country = w_app.country_option

    w_first_idea = w_app.first_idea
    w_second_idea = w_app.second_idea
    w_third_idea = w_app.third_idea

    w_WL_key_ti = w_app.WL_key_ti

    w_size_navigation = w_app.size_navigation

    w_go_button = w_app.go_button

    if w_WL_key_ti:
        WL_key = w_WL_key_ti
    elif not w_WL_key_ti:
        st.stop()

    if w_size_navigation == 'Small': x = 10
    elif w_size_navigation == 'Medium': x = 40
    elif w_size_navigation == 'Large': x = 100

    if w_go_button:

        if not (w_first_idea or w_second_idea or w_third_idea):
            ...
            if w_second_idea:
                if w_third_idea:
                    ...
                    st.success("success!")
                    keyword_list = w_first_idea + ', ' + w_second_idea + ', ' + w_third_idea
                elif not w_third_idea:
                    # if users enter only two ideas, then keyword_list will consist of only two ideas.
                    st.success("success!")
                    keyword_list = w_first_idea + ', ' + w_second_idea

            elif not (w_second_idea or w_third_idea):
